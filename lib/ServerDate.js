...
define(['config'], function (config) {
    ...
    Object.defineProperty(ServerDate, "synchronizationIntervalDelay", {
        ...
        set: function (value) {
            ...
            synchronizationInterval = setInterval(ServerDate.synchronize,
    ...
    ServerDate._setTarget = function (newTarget) {
        ...
        if (delta > config.serverDate.amortizationThreshold) {
    ...
    ServerDate.setUpConnection = function() {
        var r = new XMLHttpRequest();

        r.open("GET", config.serverDate.url);

        //if response is pending too long or network down
        r.timeout = 100;

        r.onload = function() {
            if(this.status === 200) {
                // After initialization the connection, we start first synchronization
    ...
    ServerDate.offline = function () {
        log('Network problems or request is unresolved');
        offset = new Offset(0, 0);
        ServerDate._setTarget(offset);
        synchronizing = false;
    };

    ServerDate.setUpConnection();

    // Synchronize whenever the page is shown again after losing focus.
    window.addEventListener('pageshow', ServerDate.setUpConnection);
