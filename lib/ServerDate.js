...
/*
 * Changes by Markus Haverinen
 *
 * - Added support to commonJS
 * - Added reSynchronize and fixed synchronize callback being called multiple times
 *
 */

var ServerDate = function (config, timeFunctions) {
    'use strict';
    ...
    ServerDate.synchronize = function (callback) {
        ...
        var requestSample = function () {
            ...
            request.onreadystatechange = function () {
                ...
                if ((this.readyState == (typeof this.HEADERS_RECEIVED === 'number' ? this.HEADERS_RECEIVED : 2))
                    && (this.status == 200))
        ...
        var onNetworkError = function() {
            log('Network error');
            (timeFunctions.setTimeout || setTimeout)(function() {
    ...
    synchronizeCallback = function () {
        ...
        // a small amount.
        var intervalID = (timeFunctions.setInterval || setInterval)(function () {
            if (target === null) {
                return;
            }
            ...
            if (delta !== 0) {
                ...
                log('Synchronization done with ' + offset + ' ms offset.');
                (timeFunctions.clearInterval || clearInterval)(intervalID);
                ServerDate.reSynchronize();
            }
        }, config.serverDate.amortizationInterval);
    };

    ServerDate.reSynchronize = function () {
        if (syncIntervalID) {
            (timeFunctions.clearTimeout || clearTimeout)(syncIntervalID);
        }

        log('Resynchronize at ' + (new Date(Date.now() + config.serverDate.synchronizationIntervalDelay)).toString());

        syncIntervalID = (timeFunctions.setTimeout || setTimeout)(function () {
            syncIntervalID = undefined;

            ServerDate.synchronize(synchronizeCallback);
        }, config.serverDate.synchronizationIntervalDelay);
    };
...
(function () {
    ...
    if (Function.prototype.bind) {
        var timeFunctions = {
            setTimeout : (window && window.setTimeout.bind(window)) || setTimeout,
            clearTimeout : (window && window.clearTimeout.bind(window)) || clearTimeout,
            setInterval : (window && window.setInterval.bind(window)) || setInterval,
            clearInterval : (window && window.clearInterval.bind(window)) || clearInterval
        };
    }

    var serverDateInstance,
        config,
        getServerDate = function (_config) { 
            //console.log('config', _config);
            config = _config || config;
            //console.log(config);
            serverDateInstance = serverDateInstance || ServerDate(config, timeFunctions); 
            return serverDateInstance; 
        };

    if (typeof define === 'function' && typeof define.amd === 'object' && define.amd) {
        define(['config'], getServerDate);
    } else if (freeExports && freeModule) {
        config = require('config');
        // in Node.js or RingoJS
        if (moduleExports) {
            (freeModule.exports = getServerDate).ServerDate = getServerDate;
        }
        // in Narwhal or Rhino -require
        else {
            freeExports.ServerDate = getServerDate;
        }
    }
